...
<template>
  <div class="bg-white dark:bg-appromed-dark overflow-hidden">
    <ul class="divide-y divide-appromed" v-if="orgs_to_show.length">
      <li v-for="(org, i) in orgs_to_show" :key="org.address">
        <router-link :to="org.href" class="block hover:bg-appromed-light" v-if="org.match.value > 0">
          <div class="flex items-center px-4 py-4 sm:px-6">
            <div class="min-w-0 flex-1 flex items-center">
              <div class="flex-shrink-0">
                <img class="h-12 w-12 rounded-full" :src="org.imageUrl" />
              </div>
              <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                <div>
                  <p class="text-sm font-medium text-slate-600 truncate">{{ org.name }}</p>
                  <p class="mt-2 flex items-center text-sm text-appromed">
                ...
                <div class="hidden md:block">
                  <div>
                    <p class="text-sm text-appromed-dark">
                      Match: {{ org.match.value }}%
                    </p>
                    <p v-if="i === 0" class="mt-2 flex items-center text-sm text-appromed">
